...
def extract_eml_file_observations(
    ...
    identity = _message_identity(relative_path, message_id, message)
    message_key = email_message_key(relative_path, identity["identity"])
    ...
    part_observations, part_summary = _part_observations(relative_path, message, message_key)
    thread_observations, reference_observations = _thread_observations(
        relative_path,
        message,
        message_key,
    )
    header_observations = _header_observations(relative_path, message)
    ...
    observations.append(
        _observation(
            ...
            source_id=f"{relative_path}#email-message",
            name="email message",
            target=message_key,
            metadata={
                "format": "eml",
...
def _message_identity(relative_path: str, message_id: str | None, message: Any) -> dict[str, Any]:
...
def _header_observations(relative_path: str, message: Any) -> list[RawObservation]:
    observations: list[RawObservation] = []
    for header_name in HEADER_NAMES:
        ...
        observations.append(
            _observation(
                kind="email.header",
                relative_path=relative_path,
                source_id=f"{relative_path}#email-header:{_header_slug(header_name)}",
                name=header_name,
                metadata=metadata,
            )
        )
        ...
        for display_name, address in getaddresses(values):
            ...
            observations.append(
                _observation(
                    kind="email.address",
                    relative_path=relative_path,
                    source_id=f"{relative_path}#email-address:{role}:{address_hash[:16]}",
                    name=f"{role}:{domain}",
    ...
    for index, (part, part_path, depth) in enumerate(_walk_parts(message), start=1):
        ...
        observations.append(
            _observation(
                kind="email.part",
                relative_path=relative_path,
                source_id=f"{relative_path}#email-part:{part_path}",
                name=pointer,
                target=part_key,
                metadata={
                    "format": "eml",
        ...
        if is_attachment:
            ...
            observations.append(
                _observation(
                    ...
                    relative_path=relative_path,
                    source_id=(
                        f"{relative_path}#email-attachment:"
    ...
    for ordinal, (kind, normalized_message_id) in enumerate(entries, start=1):
        ...
        hints.append(
            _observation(
                ...
                relative_path=relative_path,
                source_id=f"{relative_path}#email-thread-hint:{ordinal}",
                name=pointer,
                target=hint_key,
                metadata=metadata,
            )
        ...
        references.append(
            _observation(
                ...
                relative_path=relative_path,
                source_id=f"{relative_path}#email-reference:{kind}:{ordinal}",
                name=kind,
                target=target_key,
                metadata=metadata,
            )
        ...
        for url in _urls_in_header(value):
            ...
            observations.append(
                _observation(
                    kind="email.reference",
                    relative_path=relative_path,
                    source_id=f"{relative_path}#email-reference:list-unsubscribe:{index}",
...
def _parse_error(
    ...
    *,
    ...
    return _observation(
        ...
        relative_path=relative_path,
        source_id=f"{relative_path}#email-parse-error:{error_kind}",
        confidence="unknown",
        metadata={
            "format": "eml",
            "parser": PARSER,
