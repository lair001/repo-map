...
- Current roadmap: Phase 3 shell extractor is underway. Current discovery emits simple `shell.command`, static `shell.source`, static `shell.env`, and first-pass `shell.host_mutation` observations; `repomap-kg host-mutators <raw-observations.jsonl>` reads those raw host-mutating facts back as table or JSON. Shell facts should remain deterministic and evidence-backed.
